...
> 以下命令是系统级协议，不属于任何单一 Agent。任何 AI 会话收到这些命令都必须遵循统一执行协议。
...
**触发词**：`查看状态` / `当前状态` / `看看进度` / `查看当前状态/进度` / `status`
...
> ⚠️ **准确性要求**：迭代状态必须与实际一致。进行中不要说"刚完成"，已废弃不要说"就绪"。

### 系统重置命令

**触发词**：`执行系统重置` / `完全重置系统`

**精确匹配要求**：只有上述精确短语才触发。模糊词（"重置状态""重置一下""清理一下"）不触发，需先确认意图。

**执行协议**：详见 `docs/agent-rules/cli-operations.md#系统重置`

### 审计命令

**触发词**：`帮我审计` / `检查一致性` / `审计系统` / `audit`

**执行协议**：触发 `valveos-audit` skill
...
| 你想说 | → 去哪 |
|---------|--------|
| "执行系统重置" | 见上方「📡 系统命令#系统重置命令」 |
| "帮我审计/检查一致性" | 见上方「📡 系统命令#审计命令」 |
| "查看当前状态/进度" | 见上方「📡 系统命令#查看状态命令」 |
