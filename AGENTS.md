...
- **多个AI在同一个分支工作** - 导致51个commits、23个文件混在一起
- 分支命名与实际内容不符
...
- **每个功能/改动使用独立分支**：不要多AI共享分支
- **分支命名要准确**：`feat/xxx` 就只包含 xxx，不要混入其他功能

#### 🛡️ 多AI协作规则（避免 PR #42 的问题）
1. **不要在别人的分支上提交** - 如果需要合作，创建新分支
2. **分支是独占的** - 认领一个分支后，不要往别人的分支推代码
3. **PR 提交前 squash commits** - 把多个小commit合并成一个有意义的大commit
4. **PR description 要诚实** - 明确说明这个PR包含什么、不包含什么
