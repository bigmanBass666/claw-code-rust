...
```
用户 → Planner(决策) → Coordinator(协调) → Worker(执行) → PR Manager(质量) → Maintainer(改进) → Housekeeper(清理) → COO(系统维护)
```
...
COO 在系统维护场景下可使用**单会话模式**，在一个会话中通过 sub-agent 并行执行所有工作。

**适用**：系统文档修改、审计修复、Skill 改进
**不适用**：需要向上游提 PR 的代码修改（仍用核心流水线）

COO 每次系统迭代后必须执行：审计 → 修复 → 评估 skill → 改进 skill
