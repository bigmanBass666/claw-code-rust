...
你可以让 Agent 在唤醒后进入**待机轮询**，自动监听 inbox 而非等待你手动唤醒。

**用法**：唤醒 Agent 时附加指令："待机模式，等 [来源Agent] 消息"

**效果**：Agent 会每 5 分钟检查一次 inbox，收到消息后自动开始工作，无需你再次手动唤醒。

**限制**：终端上限 5 个，最多同时 2-3 个 Agent 待机。详见 `docs/agent-rules/cli-operations.md#待机模式`。
